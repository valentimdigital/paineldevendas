...
async function salvarCliente(dadosAPI: any, usuarioId: number) {
  try {
    ...
    if (dadosAPI.natureza_juridica) {
      ...
      if (naturezaJuridica.includes('mei') || dadosAPI.mei === true) {
        tipoEmpresa = 'MEI';
    ...
    const dadosCliente = {
      cnpj: dadosAPI.cnpj,
      nomeEmpresarial: dadosAPI.nome || dadosAPI.razao_social || '',
